...
const source = await readFile(resolve(process.cwd(), "contracts/BadgeBooth.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "BadgeBooth.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["BadgeBooth.sol"]?.BadgeBooth;
if (!contract) {
...
console.log(`Deploying BadgeBooth from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_BADGE_BOOTH_CONTRACT_ADDRESS to this contract address in Vercel.");
